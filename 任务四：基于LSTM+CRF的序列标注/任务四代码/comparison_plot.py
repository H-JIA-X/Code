...
def NN_plot(random_embedding, glove_embedding, len_feature, len_hidden, learning_rate, batch_size, iter_times):
    ...
    def cuda_to_cpu(data):
        ...
        if isinstance(data, list) and len(data) > 0 and isinstance(data[0], torch.Tensor):
            return [d.cpu().numpy() for d in data]
        return data

    # 在 NN_plot() 函数的绘图部分之前添加转换

    tra_ran = cuda_to_cpu(tra_ran)
    tra_glo = cuda_to_cpu(tra_glo)
    tea_ran = cuda_to_cpu(tea_ran)
    tea_glo = cuda_to_cpu(tea_glo)
